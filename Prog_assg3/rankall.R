rankall <- function(outcome, num = "best") {
  ...
  data <- read.csv("outcome-of-care-measures.csv" ,colClasses = "character")
    ...
    #vec_2 <-sapply(vec_2,rbind.data.frame,colnames=c("hospital" , "state")) 
    vec_2 <- cbind(do.call(rbind.data.frame, vec_2),states)
  column_names = c("hospital","state")
  names(vec_2) <- column_names 
  vec_2
   #cbind(vec_2 , state)  
